...
import { CustomerFooter } from "@/components/shared/layout/customer-footer";
import { CustomerHeader } from "@/components/shared/layout/customer-header";
import { CustomerSidebar } from "@/components/shared/layout/customer-sidebar";
...
import { resumeUserSession } from "@/lib/auth-api";
...
export function CustomerShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    async function verifySession() {
      const nextSession = await resumeUserSession();

      if (cancelled) return;

      if (!nextSession) {
        clearAuthSession();
        router.replace(USER_SIGN_IN_PATH);
        return;
      }
