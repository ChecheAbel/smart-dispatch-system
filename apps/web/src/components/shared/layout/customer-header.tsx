...
import { getCustomerPageTitleKey } from "@/lib/customer-navigation";
import {
  adminEyebrowClass,
...
export function CustomerHeader() {
  ...
  const pageTitleKey = getCustomerPageTitleKey(pathname);
  const pageTitle = copy.pageTitles[pageTitleKey];
  ...
  return (
    <header className="sticky top-0 z-20 flex h-16 shrink-0 items-center gap-3 border-b border-slate-200/80 bg-white/95 px-4 backdrop-blur supports-backdrop-filter:bg-white/90 sm:px-6">
      ...
      <div className="min-w-0 flex-1">
        <p className={adminEyebrowClass}>{copy.header.eyebrow}</p>
