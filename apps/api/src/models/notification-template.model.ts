...
const DEFAULT_TEMPLATES = [...RIDE_REQUEST_RULES, ...USER_REGISTRATION_RULES];
...
export async function listEnabledNotificationTemplates(
  module: NotificationModule,
  event: RideRequestNotificationEvent | UserRegistrationNotificationEvent,
) {
  return prisma.notificationTemplate.findMany({
    where: {
